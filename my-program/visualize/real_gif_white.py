...
parser.add_argument('-o', '--output', type=str, default='outputs/final_prezen_vis_white', help='Output directory')
...
# 軸ラベルのスタイル設定 (白背景用)
axis_label_font = dict(size=28, family='Arial Black', color='black')
...
# 軸矢印のデータ (白背景でも視認しやすい濃いめのカラー)
def create_gizmo_traces(scene_name='scene2'):
    """3軸の矢印（線 + コーン）とラベルを生成"""
    axes = [
        # (direction, color, label, label_pos) - x=Depth, y=Height, z=Width
        ([-1, 0, 0], 'rgb(220, 30, 30)', 'D', [-2.0, -0.3, 0]),
        ([0, 1, 0], 'rgb(30, 160, 30)', 'H', [0, 1.35, 0]),
        ([0, 0, 1], 'rgb(40, 80, 220)', 'W', [0, 0, 1.35]),
    ]
    traces = []
...
# サイズ情報の2Dアノテーション (白背景用に濃いめのカラー)
size_annotations = [
    dict(
        text=f'<b>D: {alphas.shape[0]}</b>', x=0.02, y=0.98,
        xref='paper', yref='paper', showarrow=False,
        font=dict(size=24, family='Arial Black', color='rgb(220, 30, 30)'),
        xanchor='left', yanchor='top',
    ),
    dict(
        text=f'<b>H: {alphas.shape[1]}</b>', x=0.02, y=0.93,
        xref='paper', yref='paper', showarrow=False,
        font=dict(size=24, family='Arial Black', color='rgb(30, 160, 30)'),
        xanchor='left', yanchor='top',
    ),
    dict(
        text=f'<b>W: {alphas.shape[2]}</b>', x=0.02, y=0.88,
        xref='paper', yref='paper', showarrow=False,
        font=dict(size=24, family='Arial Black', color='rgb(40, 80, 220)'),
        xanchor='left', yanchor='top',
    ),
]
