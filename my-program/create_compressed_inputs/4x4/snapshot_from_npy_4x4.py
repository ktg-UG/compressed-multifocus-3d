#!/usr/bin/env python3
"""
4x4ブロックごとに1つだけ開くマスキングパターンでスナップショットを生成
"""
import sys
import os
import numpy as np
...
def generate_4x4_masks(h, w, n_slices):
    """
    各4x4ブロック内で1つだけ開くマスクをn_slices分生成
    """
    ...
    assert n_slices <= block_size * block_size, "n_slicesは16以下にしてください"

    # 各sliceごとに、各ブロック内の1つのピクセルだけ1
    for s_idx in range(n_slices):
        # ブロック内のどのピクセルを開くか（0〜15）
...
def main():
    parser = argparse.ArgumentParser(description="Generate 4x4 block mask snapshot")
    ...
    masks_np = generate_4x4_masks(h, w, n_slices)
    ...
    meta = {
        ...
        'description': 'Snapshot generated with PSF convolution + 4x4 masks',
        ...
        'assignments': {
            'block_size': 4,
            'open_pixel_per_slice': 'Each slice opens a unique pixel in every 4x4 block'
        }
    }
