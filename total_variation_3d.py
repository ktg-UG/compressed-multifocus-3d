...
class TotalVariationL1Z(torch.nn.Module):
    """Calculate the total variation along the depth (z) axis only.

    Differs from TotalVariationL1 in that it omits the xy-plane terms.
    Intended to complement DIP's U-Net which already smooths xy via convolutions.
    Input shape and reduction semantics match TotalVariationL1.
    """

    def __init__(self, *, is_mean_reduction: bool = False) -> None:
        super(TotalVariationL1Z, self).__init__()
        self._is_mean = is_mean_reduction
    ...
    def forward(self, tensor_: Tensor) -> Tensor:
        ...
        if ndims_ == 3:
            ...
            raise ValueError('Input tensor must be either 3, 4 or 5-dimensional.')

        sum_axis = tuple({abs(x) for x in range(ndims_ - 4, ndims_)})
        z_denominator = (
            z_diff.shape[sum_axis[0]::].numel() if self._is_mean else 1
        )
        return torch.sum(torch.abs(z_diff), dim=sum_axis) / z_denominator
